...
def run_training_step(sess, model, fetch_summary, accuracy_to_log, feed_dict):
  ...
  model.saver.restore(sess, FLAGS.load_from_ckpt)
...
def train_ffn(model_cls, **model_kwargs):
  with tf.Graph().as_default():
    ...
    # TODO(jk: removed) with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks, merge_devices=True)):
      ...
      sv = tf.train.Supervisor(
	   ...
	   #init_fn = (lambda : saver.restore(sess, FLAGS.load_from_ckpt)))
      # saver = tf.train.Saver(var_list = tf.global_variables(), keep_checkpoint_every_n_hours=0.5)
      ...
      #     file_name=FLAGS.load_from_ckpt,
      #     tensor_name='',
      #     all_tensors=False,
      #     all_tensor_names=True)

      k=0
      while 1:#step < FLAGS.max_steps:
        if 1: # (step % 10 == 0) & (step>0):
