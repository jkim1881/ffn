...
class Canvas(object):
  ...
  def is_valid_pos(self, pos, ignore_move_threshold=False):
    ...
    if len(volume_shape)==4:
      volume_shape = volume_shape[:-1]
    if np.any(low < 0) or np.any(high >= volume_shape):
...
class Runner(object):
  ...
  def start(self, request, batch_size=1, exec_cls=None, topup=None,
    ...
    args['adabn'] = True  # Changed according to JK's suggestions True
    args['tag'] = ''
    ...
    with timer_counter(subvol_counters, 'load-image'):
      ...
      def align_and_crop(image):
        return alignment.align_and_crop(src_corner, image, dst_corner, dst_size,
                                        forward=True, with_membrane=with_membrane)
