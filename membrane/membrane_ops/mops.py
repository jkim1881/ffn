...
def initialize_tf(adabn):
    ...
    if adabn:  # hasattr(config, 'adabn') and config.adabn:
        # moving_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        # moving_ops_names = [x.name for x in moving_ops]
...
def evaluate_model(
    ...
    # Start evaluation
    sess, summary_op, summary_writer, saver, adabn_init = initialize_tf(adabn)
